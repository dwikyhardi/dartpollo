  ...
  group('FileService', () {
    group('extractBasename', () {
      test('should extract basename without extension from simple path', () {
        ...
        expect(FileService.extractBasename('query.graphql.dart'),
            equals('query.graphql'));
      });

      test('should extract basename from absolute path', () {
        expect(
            FileService.extractBasename('/path/to/file.dart'), equals('file'));
        expect(FileService.extractBasename('/usr/local/bin/app.exe'),
            equals('app'));
      });
      ...
      test('should extract basename from relative path', () {
        expect(FileService.extractBasename('./file.dart'), equals('file'));
        expect(
            FileService.extractBasename('../parent/file.dart'), equals('file'));
        expect(FileService.extractBasename('nested/deep/file.dart'),
            equals('file'));
      });
      ...
      test('should handle files without extension', () {
        ...
        expect(
            FileService.extractBasename('/path/to/LICENSE'), equals('LICENSE'));
      });

      test('should handle files with multiple dots', () {
        expect(
            FileService.extractBasename('file.test.dart'), equals('file.test'));
        expect(FileService.extractBasename('config.local.json'),
            equals('config.local'));
        expect(FileService.extractBasename('archive.tar.gz'),
            equals('archive.tar'));
      });
      ...
      test('should handle Windows-style paths', () {
        ...
        if (Platform.isWindows) {
          expect(FileService.extractBasename('C:\\Users\\file.dart'),
              equals('file'));
          expect(FileService.extractBasename('..\\parent\\file.dart'),
              equals('file'));
        } else {
    ...
    group('extractPath', () {
      test('should extract directory path from absolute path', () {
        expect(
            FileService.extractPath('/path/to/file.dart'), equals('/path/to'));
        expect(FileService.extractPath('/usr/local/bin/app'),
      ...
      test('should extract directory path from relative path', () {
        ...
        expect(FileService.extractPath('./file.dart'), equals('.'));
        expect(FileService.extractPath('../file.dart'), equals('..'));
        expect(FileService.extractPath('nested/deep/file.dart'),
            equals('nested/deep'));
      });
      ...
      test('should handle Windows-style paths', () {
        // On non-Windows platforms, backslashes are treated as part of the filename
        if (Platform.isWindows) {
          expect(FileService.extractPath('C:\\Users\\Documents\\file.dart'),
              equals('C:\\Users\\Documents'));
          expect(FileService.extractPath('..\\parent\\file.dart'),
              equals('..\\parent'));
        } else {
      ...
      test('should handle complex nested paths', () {
        expect(FileService.extractPath('a/b/c/d/e/file.dart'),
            equals('a/b/c/d/e'));
        expect(FileService.extractPath('./nested/./deep/../file.dart'),
            equals('./nested/./deep/..'));
      });
    ...
    group('validatePath', () {
      test('should pass validation for valid relative paths', () {
        ...
        expect(() => FileService.validatePath('./file.dart'), returnsNormally);
        expect(() => FileService.validatePath('../file.dart'), returnsNormally);
        expect(() => FileService.validatePath('nested/path/file.dart'),
            returnsNormally);
      });
      ...
      test(
        ...
        final tempDir =
            Directory.systemTemp.createTempSync('file_service_test');
      ...
      test(
          ...
          () {
        final nonExistentPath = '/this/path/should/not/exist/file.dart';
      ...
      test('should handle paths with special characters', () {
        expect(() => FileService.validatePath('file with spaces.dart'),
            returnsNormally);
        expect(() => FileService.validatePath('file-with-dashes.dart'),
            returnsNormally);
        expect(() => FileService.validatePath('file_with_underscores.dart'),
            returnsNormally);
        expect(() => FileService.validatePath('file.with.dots.dart'),
            returnsNormally);
      });
      ...
      test('should handle Unicode characters in paths', () {
        ...
        expect(
            () => FileService.validatePath('🚀rocket.dart'), returnsNormally);
      });
    });
    ...
    group('normalizePath', () {
      test('should normalize relative path components', () {
        ...
        expect(FileService.normalizePath('./path/../file.dart'),
            equals('file.dart'));
        expect(FileService.normalizePath('path/./file.dart'),
            equals('path/file.dart'));
        expect(FileService.normalizePath('path/../other/file.dart'),
            equals('other/file.dart'));
      });

      test('should normalize redundant separators', () {
        expect(FileService.normalizePath('path//to//file.dart'),
      ...
      test('should handle complex path normalization', () {
        expect(FileService.normalizePath('./a/../b/./c/../d/file.dart'),
            equals('b/d/file.dart'));
        expect(FileService.normalizePath('/a/b/../c/./d/../e/file.dart'),
      ...
      test('should preserve absolute vs relative nature', () {
        ...
        expect(
            FileService.normalizePath('relative/path'), isNot(startsWith('/')));
      });
    ...
    group('joinPath', () {
      test('should join simple path components', () {
        expect(FileService.joinPath(['path', 'to', 'file.dart']),
            equals('path/to/file.dart'));
      ...
      test('should handle absolute path components', () {
        expect(FileService.joinPath(['/root', 'path', 'file.dart']),
            equals('/root/path/file.dart'));
      });

      test('should handle empty components', () {
        expect(FileService.joinPath(['path', '', 'file.dart']),
            equals('path/file.dart'));
        expect(FileService.joinPath(['', 'path', 'file.dart']),
            equals('path/file.dart'));
      });
      ...
      test('should handle components with separators', () {
        expect(FileService.joinPath(['path/to', 'file.dart']),
            equals('path/to/file.dart'));
        expect(FileService.joinPath(['path', 'to/deep', 'file.dart']),
    ...
    group('isAbsolute', () {
      ...
      test('should handle Windows-style absolute paths', () {
        ...
        expect(FileService.isAbsolute('C:\\path\\to\\file'),
    ...
    group('error cases and edge conditions', () {
      ...
      test('should handle paths with spaces and special characters', () {
        final specialPath = 'path with spaces/file-name_test.dart';
        expect(
            FileService.extractBasename(specialPath), equals('file-name_test'));
        expect(
            FileService.extractPath(specialPath), equals('path with spaces'));
      ...
      test('should handle network paths and UNC paths on Windows', () {
        if (Platform.isWindows) {
          expect(
              ...
              returnsNormally);
          expect(() => FileService.extractPath('\\\\server\\share\\file.dart'),
              returnsNormally);
        }
      });
      ...
      test('should handle symbolic links and junctions gracefully', () {
        ...
        expect(() => FileService.extractBasename('symlink/file.dart'),
            returnsNormally);
        expect(() => FileService.extractPath('symlink/file.dart'),
            returnsNormally);
      });
    });
  });
