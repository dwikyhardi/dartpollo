...
void main() {
  group('AppendTypename Performance Benchmarks', () {
    ...
    group('Processing Time Benchmarks', () {
      test(
          'should measure batched vs individual AppendTypename processing time',
          () async {
        ...
        developer.log('AppendTypename Processing Time Benchmark:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Documents processed: ${documents.length}',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log(
        ...
        if (avgIndividualTime > 0) {
          expect(
              avgBatchedTime,
              lessThanOrEqualTo(avgIndividualTime *
                  10.0)); // Allow 900% tolerance for benchmark
          ...
          if (documents.length >= 300) {
            expect(
                ...
                greaterThanOrEqualTo(
                    -900.0)); // Should not be more than 900% slower
          }
        } else {
          // If individual processing is too fast to measure, just ensure batched completes
          expect(avgBatchedTime,
              lessThan(1000)); // Should complete within 1 second
        }
      });

      test('should benchmark AppendTypename with complex nested structures',
          () async {
        final documents = <DocumentNode>[];
        ...
        for (int complexity = 1; complexity <= 5; complexity++) {
          for (int i = 0; i < 20; i++) {
            documents.add(_createDeeplyNestedQuery(
                'NestedQuery${complexity}_$i', complexity));
          }
        }
        ...
        final individualTime =
            individualStopwatch.elapsedMilliseconds.toDouble();
        final batchedTime = batchedStopwatch.elapsedMilliseconds.toDouble();
        final improvement =
            (individualTime - batchedTime) / individualTime * 100;

        developer.log('Complex Nested Structures Benchmark:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Documents: ${documents.length} (5 complexity levels)',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Individual: ${individualTime.toStringAsFixed(2)}ms',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Batched: ${batchedTime.toStringAsFixed(2)}ms',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Improvement: ${improvement.toStringAsFixed(1)}%',
            level: Level.INFO.value, name: 'BENCHMARK');
        ...
        if (individualTime > 0) {
          expect(
              batchedTime,
              lessThanOrEqualTo(individualTime *
                  5.0)); // Allow 400% tolerance for complex structures
        } else {
          // If individual processing is too fast to measure, just ensure batched completes
          expect(
              batchedTime, lessThan(1000)); // Should complete within 1 second
        }
      });
      ...
      test(
        ...
        final documents = List.generate(
            100, (i) => _createComplexNestedQuery('MixedQuery$i'));
        ...
        final testCases = [
          ...
          },
          {
            ...
            'transformers': [_MockTransformer(), AppendTypename('__typename')],
          },
          {
        ...
        for (final testCase in testCases) {
          ...
          developer.log('${testCase['name']} Benchmark:',
              level: Level.INFO.value, name: 'BENCHMARK');
          developer.log('  Individual: ${individualTime}ms',
              level: Level.INFO.value, name: 'BENCHMARK');
          developer.log('  Batched: ${batchedTime}ms',
              level: Level.INFO.value, name: 'BENCHMARK');
          developer.log('  Improvement: ${improvement.toStringAsFixed(1)}%',
              level: Level.INFO.value, name: 'BENCHMARK');
          ...
          if (individualTime > 0) {
            expect(
                batchedTime,
                lessThanOrEqualTo(
                    individualTime * 5.0)); // Allow 400% tolerance
          } else {
            expect(
                batchedTime, lessThan(1000)); // Should complete within 1 second
          }
    ...
    group('Memory Usage Benchmarks', () {
      test('should measure memory usage for AppendTypename processing',
          () async {
        ...
        developer.log('AppendTypename Memory Usage Benchmark:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Documents processed: ${documents.length}',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log(
        ...
        expect(
            ...
            lessThanOrEqualTo(
                individualMemoryIncrease * 3.0)); // Allow 200% tolerance
      });

      test('should measure memory efficiency with repeated documents',
          () async {
        ...
        developer.log('Memory Efficiency with Caching Benchmark:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log(
        ...
        developer.log(
            ...
            level: Level.INFO.value,
            name: 'BENCHMARK');
        developer.log('  Cache statistics: $cacheStats',
            level: Level.INFO.value, name: 'BENCHMARK');
    ...
    group('Cache Hit Rate Analysis', () {
      test('should analyze cache hit rates for AppendTypename transformations',
          () async {
        ...
        for (int repeat = 0; repeat < 20; repeat++) {
          for (final baseDoc in baseDocuments) {
            documents.add(baseDoc);
          }
        }
        ...
        final estimatedCacheHitRate = expectedCacheHits / totalDocuments * 100;

        developer.log('Cache Hit Rate Analysis:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Total documents: $totalDocuments',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Unique documents: $uniqueDocuments',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Expected cache hits: $expectedCacheHits',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log(
        ...
        expect(stopwatch.elapsedMilliseconds,
            lessThan(500)); // Should complete quickly with caching
      });

      test('should measure cache effectiveness across multiple batches',
          () async {
        final baseDocuments =
            List.generate(5, (i) => _createComplexNestedQuery('CacheTest$i'));
        final transformers = [AppendTypename('__typename')];
        ...
        for (int batch = 0; batch < 5; batch++) {
          ...
          final stopwatch = Stopwatch()..start();
          final results =
              await batchedProcessor.processBatch(batchDocuments, transformers);
          stopwatch.stop();
        ...
        developer.log('Multi-Batch Cache Effectiveness Analysis:',
            level: Level.INFO.value, name: 'BENCHMARK');
        ...
        for (int i = 1; i < cacheStatsHistory.length; i++) {
          final prevCacheSize =
    ...
    group('Large Scale Performance Tests', () {
      test('should handle large numbers of documents efficiently', () async {
        ...
        for (final count in documentCounts) {
          ...
          developer.log('Large Scale Test - $count documents:',
              level: Level.INFO.value, name: 'BENCHMARK');
          developer.log(
        ...
        developer.log('Scalability Analysis:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Document count scaling: ${scalingFactor}x',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log(
        ...
        if (timeScalingFactor > 0 && timeScalingFactor.isFinite) {
          expect(
              timeScalingFactor,
              lessThan(scalingFactor *
                  5)); // Should not be more than 5x worse than linear
        }
        ...
        if (throughput.isFinite && throughput > 0) {
          expect(
              throughput, greaterThan(1)); // At least 1 doc/sec if measurable
        }
      });

      test('should handle complex schemas with many types efficiently',
          () async {
        ...
        for (int i = 0; i < 100; i++) {
          documents
              .add(_createWideQuery('WideQuery$i', 20)); // 20 different fields
        }

        // Create deeply nested queries
        for (int i = 0; i < 50; i++) {
          documents
              .add(_createDeeplyNestedQuery('DeepQuery$i', 6)); // 6 levels deep
        }
        ...
        developer.log('Complex Schema Performance Test:',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Documents: ${documents.length} (mixed complexity)',
            level: Level.INFO.value, name: 'BENCHMARK');
        developer.log('  Processing time: ${stopwatch.elapsedMilliseconds}ms',
            level: Level.INFO.value, name: 'BENCHMARK');
        ...
        if (throughput.isFinite) {
          expect(
              throughput,
              greaterThan(
                  10)); // At least 10 docs/sec for complex schemas if measurable
        }
      });
    });
  });
...
void _validateAppendTypenameResults(
    List<DocumentNode> documents, String typeName) {
  for (final doc in documents) {
...
void _validateOperationHasTypename(
    OperationDefinitionNode operation, String typeName) {
...
void _validateFragmentHasTypename(
    FragmentDefinitionNode fragment, String typeName) {
      ...
      selectionSet: SelectionSetNode(selections: [
        FieldNode(
          ...
          selectionSet: SelectionSetNode(selections: [
            ...
            FragmentSpreadNode(name: NameNode(value: 'UserProfileFields')),
            FieldNode(
              name: NameNode(value: 'posts'),
              selectionSet: SelectionSetNode(selections: [
