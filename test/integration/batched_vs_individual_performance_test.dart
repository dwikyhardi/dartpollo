...
void main() {
  group('Batched vs Individual Performance Tests', () {
    ...
    test(
      'should demonstrate performance improvement with large document sets',
      () async {
        ...
        dev.log(
          'Batched processing: ${batchedStopwatch.elapsedMilliseconds}ms',
        );
        dev.log(
        ...
        expect(
          batchedStopwatch.elapsedMilliseconds,
    ...
    test('should handle fragment processing efficiently', () async {
      ...
      for (var i = 0; i < 30; i++) {
        fragments.add(
          _createComplexFragment('ComplexFragment$i', 'ComplexType$i'),
        );
      }
      ...
      for (final fragment in fragments) {
        individualResults.add(
          _processFragmentIndividually(fragment, transformers),
        );
      }
      individualStopwatch.stop();
    ...
    test('should scale well with document complexity', () async {
      ...
      for (final complexity in complexityLevels) {
        final documents = List.generate(
          20,
          (i) => _createNestedQuery('Query$i', complexity),
        );
        final transformers = [AppendTypename('__typename')];
