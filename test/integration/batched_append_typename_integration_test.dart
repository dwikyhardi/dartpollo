  ...
  group('Batched AppendTypename Integration Tests', () {
    ...
    test('should handle complex nested selections identically', () async {
      ...
      final individualResult =
          _processDocumentIndividually(document, transformers);

      // Process in batch
      final batchedResults =
          await batchedProcessor.processBatch([document], transformers);
    ...
    test('should correctly add and deduplicate typename fields', () async {
      ...
      for (final result in results) {
        ...
        final typenameFields = operation.selectionSet.selections
            ...
            .toList();
        expect(typenameFields.length, equals(1),
    ...
    test('should handle custom typename field names', () async {
      ...
      final customFieldName = 'customType';
    ...
    test('should maintain performance with multiple documents', () async {
      ...
      final documents =
          List.generate(20, (index) => _createSimpleQuery('Query$index'));
      final transformers = [AppendTypename('__typename')];
    ...
    test(
      ...
      for (int i = 0; i < batchedResults.length; i++) {
        ...
        _validateNoDuplicateTypenameInDocument(batchedResults[i], '__typename');
      }
    });

    test('should correctly deduplicate typename fields in complex scenarios',
        () async {
      final documents = [
    ...
    test('should handle large batch sizes efficiently', () async {
      ...
      for (int i = 0; i < 50; i++) {
        documents.add(_createSimpleQuery('Query$i'));
        documents.add(_createComplexNestedQuery());
        documents.add(_createQueryWithFragmentSpreads());
      }
...
void _compareSelectionSets(SelectionSetNode expected, SelectionSetNode actual) {
  ...
  final expectedFragments =
      expected.selections.whereType<FragmentSpreadNode>().toList();
  final actualFragments =
      actual.selections.whereType<FragmentSpreadNode>().toList();
  expect(actualFragments.length, equals(expectedFragments.length));

  final expectedInlineFragments =
      expected.selections.whereType<InlineFragmentNode>().toList();
  final actualInlineFragments =
      actual.selections.whereType<InlineFragmentNode>().toList();
...
void _validateTypenameFieldsInDocument(
    DocumentNode document, String typeNameField) {
  for (final definition in document.definitions) {
    if (definition is OperationDefinitionNode) {
      _validateTypenameFieldsInSelectionSet(
          definition.selectionSet, typeNameField);
    } else if (definition is FragmentDefinitionNode) {
      _validateTypenameFieldsInSelectionSet(
          definition.selectionSet, typeNameField);
    }
...
void _validateTypenameFieldsInSelectionSet(
  ...
  if (selectionSet.selections.isNotEmpty) {
    final hasTypenameField = selectionSet.selections
        .whereType<FieldNode>()
        .any((field) => field.name.value == typeNameField);

    expect(hasTypenameField, isTrue,
        reason:
...
void _validateNoDuplicateTypenameInFragment(
    FragmentDefinitionNode fragment, String typeNameField) {
  _validateNoDuplicateTypenameInSelectionSet(
      fragment.selectionSet, typeNameField);
}
...
class _MockTransformer extends TransformingVisitor {
  ...
  OperationDefinitionNode visitOperationDefinitionNode(
    ...
    final mockField = FieldNode(name: NameNode(value: '__mockField'));
...
FragmentDefinitionNode _createNestedFragment(
    [String name = 'NestedFragment', String type = 'User']) {
  return FragmentDefinitionNode(
    name: NameNode(value: name),
    typeCondition: TypeConditionNode(
      on: NamedTypeNode(name: NameNode(value: type)),
          ...
          selectionSet: SelectionSetNode(selections: [
            ...
            FieldNode(
              ...
              selectionSet: SelectionSetNode(selections: [
                ...
                FieldNode(
                    name:
                        NameNode(value: '__typename')), // Already has typename
              ]),
            ),
          ]),
        ),
    ...
    OperationDefinitionNode(
      ...
      selectionSet: SelectionSetNode(selections: [
        ...
        FieldNode(
          ...
          selectionSet: SelectionSetNode(selections: [
            FieldNode(name: NameNode(value: 'id')),
            FieldNode(name: NameNode(value: '__typename')), // Has typename
          ]),
        ),
      ]),
    ),
    ...
    OperationDefinitionNode(
      ...
      selectionSet: SelectionSetNode(selections: [
        ...
        FieldNode(
          ...
          selectionSet: SelectionSetNode(selections: [
            FieldNode(name: NameNode(value: 'id')),
            FieldNode(name: NameNode(value: '__typename')), // User has typename
            FieldNode(
              name: NameNode(value: 'posts'),
              selectionSet: SelectionSetNode(selections: [
                FieldNode(name: NameNode(value: 'title')),
