...
String _generateOptimizedDocumentNode(DocumentNode document) {
  final buffer = StringBuffer()..writeln('DocumentNodeHelpers.document([');

  for (final definition in document.definitions) {
    if (definition is FragmentDefinitionNode) {
      buffer
        ..writeln('  DocumentNodeHelpers.fragmentDefinition(')
        ..writeln("    '${definition.name.value}',")
        ..writeln("    '${definition.typeCondition.on.name.value}',");
      ...
      buffer
        ..writeln('  DocumentNodeHelpers.operation(')
        ..writeln('    OperationType.${definition.type.name},')
        ..writeln("    '${definition.name?.value ?? ''}',");
      ...
      if (definition.variableDefinitions.isNotEmpty) {
        ...
        for (final varDef in definition.variableDefinitions) {
          buffer
            ..writeln('      VariableDefinitionNode(')
            ..writeln(
              '        variable: DocumentNodeHelpers.variable(\'${varDef.variable.name.value}\'),',
            )
            ..writeln(
              '        type: ${_generateTypeNodeCode(varDef.type)},',
            );
...
void _writeOptimizedSelection(
  ...
  if (selection is FieldNode) {
    ...
    if (selection.arguments.isNotEmpty) {
      buffer.write(', args: {');
      for (var i = 0; i < selection.arguments.length; i++) {
    ...
    buffer.write(
      '${indentStr}DocumentNodeHelpers.inlineFragment(\'$typeName\'',
    );
